...
class Visualizer:
    ...
    def visualize(self, driving, source, out):
        ...
        if 'occlusion_map' in out:
            ...
            occlusion_map = F.interpolate(occlusion_map, size=source.shape[1:3] , align_corners=True).numpy()
        ...
        if 'sparse_deformed' in out:
            ...
            for i in range(out['sparse_deformed'].shape[1]):
                ...
                image = F.interpolate(image, size=source.shape[1:3], align_corners=True)
                mask = out['mask'][:, i:(i+1)].data.cpu().repeat(1, 3, 1, 1)
                mask = F.interpolate(mask, size=source.shape[1:3], align_corners=True)
